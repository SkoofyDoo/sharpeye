...
class GatesConfig(BaseModel):
    rules: list[GateRule] = Field(default_factory = dict)

class ScoringConfig(BaseModel):
    wieghts: dict[str, float] = Field(default_factory = dict)
    good_percentile: float = Field(default = 0.67, ge = 0.0, le = 1.0) 
    medium_percentile: float = Field(default = 0.67, ge = 0.0, le = 1.0) 
